...
// STREAMING-K single dot: cb_a/cb_b are SHALLOW ring buffers; the reader streams pair k
// just-in-time (1 a-tile + 1 b-tile per k), so K is unbounded by L1. Per k: wait the streamed
// pair (tile 0), compute its contribution, pop the pair, accumulate. Quire seeded compute-side.
...
void kernel_main() {
    ...
    const uint32_t k_len = get_arg_val<uint32_t>(0);

    init_sfpu(cb_a, cb_C);
    cb_wait_front(cb_zero, 1);
    seed_qzero(cb_zero, cb_qA);   // q = 0

    for (uint32_t k = 0; k < k_len; ++k) {
        const auto cb_qin = (k & 1u) ? cb_qB : cb_qA;
        const auto cb_qout = (k & 1u) ? cb_qA : cb_qB;
        cb_wait_front(cb_a, 1);       // streamed pair k
        cb_wait_front(cb_b, 1);
        cb_wait_front(cb_qin, 8);
        compute_C(cb_a, cb_b, cb_C, /*tile=*/0);   // tile 0 of the streamed ring buffers
        cb_pop_front(cb_a, 1);
        cb_pop_front(cb_b, 1);
