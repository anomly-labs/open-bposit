...
void kernel_main() {
    constexpr auto cb_a = tt::CBIndex::c_0;       // code_a (1 tile)
    constexpr auto cb_b = tt::CBIndex::c_1;       // code_b (1 tile)
    constexpr auto cb_zero = tt::CBIndex::c_2;    // carry seed (1)
    constexpr auto cb_carry_mid = tt::CBIndex::c_3;
    constexpr auto cb_qin = tt::CBIndex::c_4;     // running quire in (8)
    constexpr auto cb_C = tt::CBIndex::c_5;       // signed contribution (8) — phase1->phase2 handoff
    constexpr auto cb_qout = tt::CBIndex::c_16;   // running quire out (8)

    init_sfpu(cb_a, cb_C);
    cb_wait_front(cb_a, 1);
    cb_wait_front(cb_b, 1);

    // ---- PHASE 1: C = signed contribution of a*b (place + 1-limb negate), -> cb_C (8 tiles) ----
    ...
    // ---- PHASE 2: q_out = q_in + C  (q256_add, 4+4 leases + carry-mid CB) ----
