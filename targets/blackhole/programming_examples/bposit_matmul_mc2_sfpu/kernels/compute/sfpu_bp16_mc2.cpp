...
void kernel_main() {
    ...
    init_sfpu(cb_a, cb_C);
    cb_wait_front(cb_b, k_len);   // W: resident for all rows (read, never popped)
    cb_wait_front(cb_zero, 1);

    for (uint32_t r = 0; r < n_rows; ++r) {
        seed_qzero(cb_zero, cb_qA);   // q = 0 for this row (compute-side -> cb_qA is ping-pong-only)
        cb_wait_front(cb_a, k_len);   // this row's A
        for (uint32_t k = 0; k < k_len; ++k) {
            const auto cb_qin = (k & 1u) ? cb_qB : cb_qA;
            const auto cb_qout = (k & 1u) ? cb_qA : cb_qB;
            cb_wait_front(cb_qin, 8);
            compute_C(cb_a, cb_b, cb_C, k);
        ...
        copy_q(cb_final, cb_out);      // -> cb_out (8) for the writer
        cb_pop_front(cb_final, 8);
        cb_pop_front(cb_a, k_len);     // free this row's A
    }
