...
//
// bposit_matmul_mc2_sfpu — ARBITRARY-M multi-core exact-quire bp16 MATMUL. Removes the
...
static constexpr uint32_t M = 256;   // output rows (CAN exceed core count; multi-row-per-core)
...
int main() {
    ...
    auto reader = CreateKernel(
        program, OVERRIDE_KERNEL_PREFIX "bposit_matmul_mc2_sfpu/kernels/dataflow/read_mc2.cpp", all_cores,
    ...
    auto writer = CreateKernel(
        program, OVERRIDE_KERNEL_PREFIX "bposit_matmul_mc2_sfpu/kernels/dataflow/write_mc2.cpp", all_cores,
        DataMovementConfig{.processor = DataMovementProcessor::RISCV_1, .noc = NOC::RISCV_1_default, .compile_args = wargs});
    auto compute = CreateKernel(
        program, OVERRIDE_KERNEL_PREFIX "bposit_matmul_mc2_sfpu/kernels/compute/sfpu_bp16_mc2.cpp", all_cores,
    ...
    if (pass) {
        std::cout << "PASS: ARBITRARY-M multi-core exact-quire MATMUL, " << (total - bad) << "/" << total
        ...
        std::cout << "FAIL: ARBITRARY-M multi-core exact-quire MATMUL, " << (total - bad) << "/" << total
