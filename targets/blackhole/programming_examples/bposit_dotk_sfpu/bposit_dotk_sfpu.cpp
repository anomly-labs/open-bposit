...
//
// bposit_dotk_sfpu — in-kernel-K-loop exact bp16 dot on a PHYSICAL Tenstorrent Blackhole.
// ONE dispatch accumulates the full length-K dot per lane (q[8] += a_k*b_k), keeping the
// running quire resident in L1 via a ping-pong CB — no per-k dispatch overhead. Bit-exact
// vs the scalar bp16_madd_q256 chain; also wall-clocks the single dispatch for a per-MAC
// throughput estimate (32 lanes * K MACs in one dispatch). Kernel: sfpu_bp16_dotk.cpp.
...
int main() {
    ...
    make_cb(CBIndex::c_0, K);       // all a-codes
    make_cb(CBIndex::c_1, K);       // all b-codes
    make_cb(CBIndex::c_2, 1);       // zero
    make_cb(CBIndex::c_3, 1);       // carry_mid
    make_cb(CBIndex::c_5, N_LIMB);  // C
    make_cb(CBIndex::c_6, N_LIMB);  // qA
    make_cb(CBIndex::c_7, N_LIMB);  // qB
    make_cb(CBIndex::c_16, N_LIMB); // out
    ...
    const uint64_t macs = (uint64_t)LANES * K;
    if (pass) {
        std::cout << "PASS: in-kernel-K-loop bp16 exact dot (K=" << K << "), " << (LANES - bad) << "/" << LANES
                  << " lanes bit-exact (8-limb quire)\n";
        std::cout << "  [perf] 1 dispatch, " << macs << " exact-quire MACs (" << LANES << " lanes x " << K
                  << "), wall " << std::fixed << std::setprecision(1) << us << " us = "
                  << std::setprecision(3) << (us * 1000.0 / (double)macs) << " ns/MAC (incl. 1 dispatch + DRAM I/O)\n";
    } else {
        std::cout << "FAIL: in-kernel-K-loop bp16 exact dot (K=" << K << "), " << (LANES - bad) << "/" << LANES
