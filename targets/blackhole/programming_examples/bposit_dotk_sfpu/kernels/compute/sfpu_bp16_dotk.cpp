...
void kernel_main() {
    // IN-KERNEL K-LOOP: one dispatch accumulates the whole length-K dot per lane, keeping the
    // running quire resident in L1 (ping-pong cb_qA/cb_qB) -> no per-k dispatch overhead.
    constexpr auto cb_a = tt::CBIndex::c_0;       // all K a-codes (K tiles)
    constexpr auto cb_b = tt::CBIndex::c_1;       // all K b-codes (K tiles)
    constexpr auto cb_zero = tt::CBIndex::c_2;    // carry seed (1)
    constexpr auto cb_carry_mid = tt::CBIndex::c_3;
    constexpr auto cb_C = tt::CBIndex::c_5;       // per-k signed contribution (8)
    constexpr auto cb_qA = tt::CBIndex::c_6;      // running quire ping (8)
    constexpr auto cb_qB = tt::CBIndex::c_7;      // running quire pong (8)
    constexpr auto cb_out = tt::CBIndex::c_16;    // final quire (8)

    const uint32_t k_len = get_arg_val<uint32_t>(0);  // K

    init_sfpu(cb_a, cb_C);
    cb_wait_front(cb_a, k_len);
    cb_wait_front(cb_b, k_len);
    cb_wait_front(cb_zero, 1);

    // cb_qA seeded with q=0 (8 tiles) by the reader.
    for (uint32_t k = 0; k < k_len; ++k) {
        const auto cb_qin = (k & 1u) ? cb_qB : cb_qA;
        const auto cb_qout = (k & 1u) ? cb_qA : cb_qB;
        cb_wait_front(cb_qin, 8);
        compute_C(cb_a, cb_b, cb_C, k);              // -> cb_C (8)
        cb_wait_front(cb_C, 8);
        quire_add(cb_qin, cb_C, cb_qout, cb_zero, cb_carry_mid);  // q_out = q_in + C
        cb_pop_front(cb_C, 8);
        cb_pop_front(cb_qin, 8);
    }
    // final running quire is in cb_qA if k_len even, else cb_qB -> copy to cb_out.
    const auto cb_final = (k_len & 1u) ? cb_qB : cb_qA;
    copy_q(cb_final, cb_out);
    cb_pop_front(cb_final, 8);

    cb_pop_front(cb_a, k_len);
    cb_pop_front(cb_b, k_len);
