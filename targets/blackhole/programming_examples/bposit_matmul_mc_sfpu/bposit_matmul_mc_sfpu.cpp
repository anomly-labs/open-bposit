...
//
// bposit_matmul_mc_sfpu — MULTI-CORE exact-quire bp16 MATMUL across the Blackhole grid.
// Each Tensix core computes ONE output row Y[m][:] = sum_k X[m][k]*W[k][n] (n = the 32 SFPU
// lanes) via the in-kernel-K-loop exact-quire dot — reusing the bposit_dotk_sfpu COMPUTE
// kernel VERBATIM. M rows fan out across M cores -> M*32 exact-quire dots in ONE dispatch.
// Gated bit-exact (M*32*8 limbs) vs the scalar bp16_madd_q256 chain; reports aggregate
// MACs/s across the grid (honest scaling vs the single-core path).
...
static constexpr uint32_t M_TARGET = 130;   // output rows (clamped to grid size; 1 row/core)
...
int main() {
    ...
    const uint32_t M = std::min(M_TARGET, num_cores_avail);   // one row per core

    // X (M x K), W (K x N=32), mixed signs.
    ...
    auto a_dram = mk(M * K);   // X[m][k] broadcast: page m*K+k
    auto b_dram = mk(K);       // W[k][n]: page k
    auto q_dram = mk(M * N_LIMB);
    auto zero_dram = mk(1);
    ...
    // distribute M rows over the grid (1 row/core since M <= num_cores)
    ...
    auto reader = CreateKernel(
        program, OVERRIDE_KERNEL_PREFIX "bposit_matmul_mc_sfpu/kernels/dataflow/read_dotk_mc.cpp", all_cores,
    ...
    auto writer = CreateKernel(
        program, OVERRIDE_KERNEL_PREFIX "bposit_matmul_mc_sfpu/kernels/dataflow/write_qk_mc.cpp", all_cores,
        DataMovementConfig{.processor = DataMovementProcessor::RISCV_1, .noc = NOC::RISCV_1_default, .compile_args = wargs});
    auto compute = CreateKernel(
        program, OVERRIDE_KERNEL_PREFIX "bposit_dotk_sfpu/kernels/compute/sfpu_bp16_dotk.cpp", all_cores,
        ComputeConfig{.fp32_dest_acc_en = true, .dst_full_sync_en = true});

    // per-core runtime args: each core owns row m = its flat work index.
    uint32_t row = 0;
            ...
            for (const auto& core : range) {
                SetRuntimeArgs(program, reader, core,
                               {(uint32_t)a_dram->address(), (uint32_t)b_dram->address(),
                                (uint32_t)zero_dram->address(), K, row * K});
                SetRuntimeArgs(program, writer, core, {(uint32_t)q_dram->address(), row * N_LIMB});
                SetRuntimeArgs(program, compute, core, {K});
                row += per;  // per == 1 (M <= num_cores)
            }
    }
    ...
    std::cout << "  grid " << grid.x << "x" << grid.y << " (" << num_cores_avail << " cores), used "
              << num_cores << " cores = " << M << " rows x " << LANES << " cols, K=" << K << "\n";
    if (pass) {
        std::cout << "PASS: MULTI-CORE exact-quire bp16 MATMUL, " << (total - bad) << "/" << total
                  << " output elements bit-exact (8-limb quire) across " << num_cores << " cores\n";
        std::cout << "  [perf] " << macs << " exact MACs in 1 dispatch, wall " << std::fixed
                  << std::setprecision(1) << us << " us = " << std::setprecision(1)
                  << ((double)macs / us) << " MAC/us aggregate (incl. dispatch + DRAM I/O); "
                  << num_cores << "-core x 32-lane = " << (num_cores * 32) << "-wide exact accumulation\n";
    } else {
        std::cout << "FAIL: MULTI-CORE exact-quire bp16 MATMUL, " << (total - bad) << "/" << total
