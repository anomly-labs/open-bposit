...
//
// read_dotk_mc.cpp — multi-core reader for the SFPU exact-quire matmul. This core owns ONE
// output row m: it loads row m's K a-tiles (a_dram pages [m*K .. m*K+K-1]) into cb_a, the
// SHARED K weight tiles (b_dram pages [0..K-1]) into cb_b, the zero carry-seed, and seeds the
// running-quire ping buffer cb_qA with 8 zero tiles. Same compute kernel as bposit_dotk_sfpu.
...
void kernel_main() {
    ...
    uint32_t a_base = get_arg_val<uint32_t>(4);   // = m * K (this core's row offset into a_dram)
    ...
    for (uint32_t k = 0; k < k_len; ++k) noc_async_read_page(a_base + k, a, awr + k * tile_bytes);
