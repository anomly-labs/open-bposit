...
// across MANY Blackhole baby (RISC-V) cores, bit-exact vs the scalar reference kernels (targets/blackhole/kernel)
...
//   - programming_examples/bposit_quire_reduce/bposit_quire_reduce.cpp  (run_matmul host path, golden gate, host
//   self-check)
//   - programming_examples/vecadd_multi_core/vecadd_multi_core.cpp      (CoreRange grid, split_work_to_cores, per-core
//   SetRuntimeArgs)
...
// reuse the SAME golden as the proven single-big-page multicore example, so a
// PASS here means the ttnn ROW_MAJOR interleaved layout reproduces it bit-exact.
#include "../bposit_quire_matmul_multicore/quire_matmul_mc_golden_cases.h"
...
int main(int /*argc*/, char** /*argv*/) {
    ...
    for (int i = 0; i < A_COUNT; ++i) {
        a_words[i] = (uint32_t)(QGOLD_MMMC_A_CODES[i] & 0xFFFF);
    }
    for (int i = 0; i < B_COUNT; ++i) {
        b_words[i] = (uint32_t)(QGOLD_MMMC_B_CODES[i] & 0xFFFF);
    }
    ...
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < N; ++j) {
            ...
            for (int k = 0; k < K; ++k) {
                ...
                if (per != baked) {
                    per_elem_ok = false;
                    std::cout << "  prod[" << i << "," << k << "," << j << "] exact=0x" << std::hex << per
                              << " baked-mul=0x" << baked << std::dec << "\n";
                }
            }
            for (int k = K - 1; k >= 0; --k) {  // reverse-order accumulation
                unsigned c[QLIMBS];
                bp16_prod_to_q256((int)(a_words[i * K + k] & 0xFFFF), (int)(b_words[k * N + j] & 0xFFFF), c);
                q256_add(q_rev, c);
            }
            ...
            if (ref_readout != (QGOLD_MMMC_C_READOUT[i * N + j] & 0xFFFF)) {
                readout_ok = false;
            }
            if (i == GATE_I && j == GATE_J) {
                for (int l = 0; l < QLIMBS; ++l) {
                    if (q[l] != QGOLD_MMMC_GATE_QUIRE[l]) {
                        gate_ok = false;
                    }
                }
            }
    ...
    if (!(per_elem_ok && readout_ok && assoc_ok && gate_ok)) {
        std::cout << "FAIL: baked MULTI-CORE MATMUL golden disagrees with the reused "
                     ...
                     "oracle (per_elem="
                  << per_elem_ok << " readout=" << readout_ok << " assoc=" << assoc_ok << " gate=" << gate_ok << ")\n";
        return 1;
    }
    ...
    // ttnn ROW_MAJOR uint32 layout: A=M row-pages (K words each), B=K row-pages
    // (N words), C=M row-pages (N words) — interleaved across DRAM banks, exactly
    // what a normal ttnn ROW_MAJOR tensor produces. gate: one 8-word page.
    // ttnn ROW_MAJOR layout, respecting Blackhole's 64-byte DRAM granularity:
    //   A: M row-pages of K=16 codes = 64 bytes/row  -> row-paged (>=64B, exact)
    //   B: K rows of N=8 codes = 32 bytes/row (<64B)  -> single page (bulk fallback)
    //   C: M row-pages of N=8 codes = 32 bytes/row written as ONE aligned full-row
    //      write per core (full-page write is 16B-aligned even though a row is 32B).
    // Real-model dims (N,K >= 16) make every row >= 64B, so all three are row-paged.
    auto a_dram = mk_paged(a_bytes, (size_t)K * WORD, BufferType::DRAM);  // M row-pages (64B/row)
    auto b_dram = mk_paged(b_bytes, b_bytes, BufferType::DRAM);           // single page (32B rows < 64B)
    auto c_dram = mk_paged(c_bytes, (size_t)N * WORD, BufferType::DRAM);  // M row-pages (ttnn ROW_MAJOR)
    ...
    auto [num_cores, all_cores, core_group_1, core_group_2, outs_per_core_g1, outs_per_core_g2] =
        tt::tt_metal::split_work_to_cores(
            core_grid, (uint32_t)M, /*row_major=*/true);  // split by ROW (ttnn row-paged writes)

    // ONE CreateKernel over the whole used core range — same proven scalar kernel.
    auto kernel = CreateKernel(
        program,
        OVERRIDE_KERNEL_PREFIX "bposit_quire_matmul_ttnn/kernels/quire_matmul_ttnn_baby.cpp",
        all_cores,
        DataMovementConfig{.processor = DataMovementProcessor::RISCV_0, .noc = NOC::RISCV_0_default});

    // Per-core runtime args: each core gets a disjoint [out_start,out_end) slice.
    auto work_groups = {std::make_pair(core_group_1, outs_per_core_g1), std::make_pair(core_group_2, outs_per_core_g2)};
        ...
        for (const auto& range : group.ranges()) {
            for (const auto& core : range) {
                ...
                SetRuntimeArgs(
                    ...
                    kernel,
                    core,
                    {(uint32_t)a_dram->address(),
                     (uint32_t)b_dram->address(),
                     (uint32_t)c_dram->address(),
                     (uint32_t)a_l1->address(),
                     (uint32_t)b_l1->address(),
                     (uint32_t)c_l1->address(),
                     (uint32_t)M,
                     (uint32_t)K,
                     (uint32_t)N,
                     out_start,
    ...
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < N; ++j) {
            ...
            if (dev != gold) {
                if (++bad_c <= 16) {
                    std::cout << "  C[" << i << "," << j << "] device=0x" << std::hex << dev << " golden=0x" << gold
                              << std::dec << "\n";
                }
            }
        }
    ...
    for (int i = 0; i < QLIMBS; ++i) {
        dev_q[i] = gate_result[i];
    }
    int bad_q = 0;
    for (int i = 0; i < QLIMBS; ++i) {
        if (dev_q[i] != QGOLD_MMMC_GATE_QUIRE[i]) {
            if (++bad_q <= 8) {
                std::cout << "  gate limb[" << i << "] device=0x" << std::hex << dev_q[i] << " golden=0x"
                          << QGOLD_MMMC_GATE_QUIRE[i] << std::dec << "\n";
            }
        }
    }
    ...
    std::cout << "[mm-ttnn] cores used = " << num_cores << " (grid " << core_grid.x << "x" << core_grid.y
              << "), outputs = " << C_COUNT << " (one+ per core)\n";
    std::cout << "[mm-ttnn] device C readouts (row-major):";
    ...
    std::cout << std::dec << "\n[mm-ttnn] golden C readouts (row-major):";
    for (int o = 0; o < C_COUNT; ++o) {
        if (o % N == 0) {
            std::cout << "\n   ";
        }
    ...
    std::cout << std::dec << "\n";
    std::cout << "[mm-ttnn] device gate(" << GATE_I << "," << GATE_J << ") quire = " << quire_to_hex_le(dev_q) << "\n";
    std::cout << "[mm-ttnn] golden gate(" << GATE_I << "," << GATE_J << ") quire = " << QGOLD_MMMC_GATE_QUIRE_HEX
              << "\n";
    std::cout << (pass ? "PASS" : "FAIL") << ": exact bp16 quire MATMUL (" << M << "x" << K << "x" << N << ") across "
              << num_cores << " cores on Blackhole, " << matched << "/" << total_checks << " match golden\n";
